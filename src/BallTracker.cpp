...
public:
    ...
    : it_(nh_)
  {
    ...
    image_sub_ = it_.subscribe("/camera/rgb/image_color", 1, &BallTracker::findBall, this);

    mode_change = nh_.subscribe("/mode_change", 1, &BallTracker::change_mode, this);
...
void findBall(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    IplConvKernel * erodeSE = cvCreateStructuringElementEx(3,3,0,0,CV_SHAPE_ELLIPSE, NULL);
    IplConvKernel * dilateSE = cvCreateStructuringElementEx(3,3,2,2,CV_SHAPE_ELLIPSE, NULL);
	...
	cvCvtColor(&static_cast<IplImage>(cv_ptr->image), hsv_frame, CV_BGR2HSV);
	...
	cvCircle( &static_cast<IplImage>(cv_ptr->image), cvPoint(cvRound(x),cvRound(y)),
							 3, CV_RGB(0,255,0), -1, 8, 0 );

	// Publish the ball info
	if (x==x){//not NAN
		if (consecuvite_counter>=30){
			robo_dog::ballinfo b;
			b.x = x;
			b.y = y;
